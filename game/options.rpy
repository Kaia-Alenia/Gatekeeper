...
define config.name = _("GateKeeper")
...
define config.save_directory = "GateKeeper"
...
init python:
    ...
    # Eliminamos la 's' de la lista de teclas para captura
    config.keymap['screenshot'] = [ 'alt_K_s', 'K_F12', 'meta_shift_K_s' ]
    ## Para archivar, se clasifican como 'archive'.
    
    # 1. Declaramos el archivo contenedor (se llamará 'archive.rpa')
    build.archive("archive", "all")

    # 2. Scripts y código (oculta la lógica del juego)
    build.classify('game/**.rpy', 'archive')
    build.classify('game/**.rpyc', 'archive')

    # 3. Multimedia (oculta imágenes, música y sonidos)
    build.classify('game/**.png', 'archive')
    build.classify('game/**.jpg', 'archive')
    build.classify('game/**.jxl', 'archive')
    build.classify('game/**.webp', 'archive')
    build.classify('game/**.ogg', 'archive')
    build.classify('game/**.mp3', 'archive')
    build.classify('game/**.wav', 'archive')
    # Esto meterá el icono de la ventana dentro del archivo .rpa
    build.classify('game/gui/**', 'archive')
    # 4. Traducciones y fuentes (oculta la carpeta tl y fuentes de texto)
    build.classify('game/tl/**', 'archive')
    build.classify('game/**.ttf', 'archive')
    build.classify('game/**.otf', 'archive')

    # 5. Interfaz de usuario (todo lo que esté en la carpeta gui)
    build.classify('game/gui/**', 'archive')
    ...
    build.executable_icon = "icon.ico"
